...
tf.app.flags.DEFINE_string("word_embedding_file", 'wiki_zh_word2vec/wiki.zh.text.vector', "extra word embeddings.")
tf.app.flags.DEFINE_string("model_path", 'resource/model/', "model save path")
# 这里默认词向量的维度是300, 如果自行训练的词向量维度不是300,则需要该这里的值。
tf.app.flags.DEFINE_integer("word_vec_size", 400000, "Size of word vector.")
tf.app.flags.DEFINE_integer("embeddings_size", 300, "Size of word embedding.")
tf.app.flags.DEFINE_integer("max_sequence", 100, "max sequence length.")  # 100

tf.app.flags.DEFINE_integer("batch_size", 128, "batch size.")  # 128
tf.app.flags.DEFINE_integer("epoch", 1000, "epoch.")  # 10000
