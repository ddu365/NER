...
word_vec_size = config.FLAGS.word_vec_size
...
def build_word_index():
    ...
    if not os.path.exists(src_vocab_file):
        with open(src_vocab_file, 'w', encoding='utf-8') as source:
            with open(word_embedding_file, 'r', encoding='utf-8') as f:
                for _ in range(word_vec_size + 1):
                    line = f.readline()  # 大文件一行行读
                    if _ > 0:
                        values = line.split()
                        word = values[0]  # 取词
                        source.write(word + '\n')
    ...
    if not os.path.exists(tgt_vocab_file):
        with open(tgt_file, 'r', encoding='utf-8') as source:
            ...
            for line in source.readlines():  # 小文件全部加载到内存
                line = line.strip()
                if line != '':
                    word_arr = line.split()
                    for w in word_arr:
...
def get_predict_iterator(src_vocab_table, vocab_size, batch_size, max_len=max_sequence):
    ...
    # WAHTEVER = 10
    fake_tag = tf.placeholder(tf.int32, [None, None])  # WAHTEVER
    return BatchedInput(
        initializer=batched_iter.initializer,
        source=src_ids,
...
def load_word2vec_embedding(vocab_size):
    ...
    with open(word_embedding_file, encoding='utf-8') as f:
        for index, line in enumerate(f):
            if index > 0 and index <= word_vec_size + 1:
                values = line.split()
                try:
                ...
                embeddings[index - 1] = coefs   # 将词和对应的向量存到字典里
